...
switch($method) {
    ...
    case 'POST':
        ...
        // Kiểm tra xem có gửi dữ liệu tạo đơn hàng không (có action=create hoặc có field subtotal/total)
        if (isset($data['cart']) || isset($data['total'])) {
            
            if (empty($data['cart'])) {
                http_response_code(400);
                echo json_encode(["status" => "error", "message" => "Giỏ hàng trống! Vui lòng thêm sản phẩm."]);
                break;
            }
